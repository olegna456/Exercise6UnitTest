...
class ReadWrite {
	...
	static ArrayList<String> readFile(int row, int column) throws IOException {
		FileReader file_to_read = new FileReader("D:/maven2final/maventwo/txtfile/test.txt");
	...
	static void writeToFile(List<String> rowList, List<String> columnList, int row, int column) throws IOException {
		...
		BufferedWriter bw =  new BufferedWriter(new FileWriter("D:/maven2final/maventwo/txtfile/test.txt"));
		try {
			while(rowCtr < row) {
				columnCtr = 0;
